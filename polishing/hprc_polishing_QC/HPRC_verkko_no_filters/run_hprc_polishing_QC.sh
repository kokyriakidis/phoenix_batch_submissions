mkdir /private/groups/migalab/kkyriaki/hprc_polishing/hprc_int_asm/HPRC_verkko_no_filters
cd /private/groups/migalab/kkyriaki/hprc_polishing/hprc_int_asm/HPRC_verkko_no_filters

git -C /private/groups/migalab/kkyriaki/phoenix_batch_submissions pull

cp -r /private/groups/migalab/kkyriaki/phoenix_batch_submissions/polishing/hprc_polishing_QC/HPRC_verkko_no_filters/* ./

# polish assemblies with unfiltered vcf
cd /private/groups/migalab/kkyriaki/hprc_polishing/hprc_deepPolisher_wf_runs/phoenix_batch_submissions_manuscript/hprc_verkko/

# filter files to pass only, polish raw assemblies
ls | grep "^HG" | while read line; do
    mat_fa=`grep ${line} /private/groups/migalab/kkyriaki/hprc_polishing/hprc_int_asm/HPRC_verkko_no_filters/hprc_verkko_hprc_deepPolisher.csv | cut -f14 -d","`
    pat_fa=`grep ${line} /private/groups/migalab/kkyriaki/hprc_polishing/hprc_int_asm/HPRC_verkko_no_filters/hprc_verkko_hprc_deepPolisher.csv | cut -f13 -d","`
    echo ${line}
...
cut -f1 -d"," /private/groups/migalab/kkyriaki/hprc_polishing/hprc_int_asm/HPRC_verkko_no_filters/hprc_verkko_hprc_deepPolisher.csv | grep -v "sample_id" | while read line; do
...
cd /Users/kokyriakidis/Desktop/Paten_lab/phoenix_batch_submissions/polishing/hprc_polishing_QC/HPRC_verkko_no_filters/hprc_polishing_QC_input_jsons
